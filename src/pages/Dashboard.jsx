      ...
      <div className="p-10 max-w-7xl mx-auto space-y-16">
        ...
        <header className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 pt-6">
          <div className="space-y-4">
            <h1 className="text-7xl font-black text-text-primary tracking-tighter leading-none">
              {currentWorkspace ? currentWorkspace.name : 'Welcome back!'}
            </h1>
            <p className="text-xl font-medium text-text-tertiary max-w-2xl leading-relaxed">
              {currentWorkspace
          ...
          {!workspaceSlug && (
            <div className="flex items-center gap-4">
              ...
              <button
                className="px-8 py-4 bg-brand-primary text-white rounded-[24px] font-black uppercase tracking-widest text-sm shadow-2xl shadow-brand-primary/20 hover:scale-105 active:scale-95 transition-all flex items-center gap-3"
            ...
            <div className="lg:col-span-8 space-y-16">
              ...
              <section>
                <div className="flex items-center justify-between mb-10">
                  <h2 className="text-xs font-black text-text-tertiary uppercase tracking-[0.3em]">Recently Viewed</h2>
                  <button 
              ...
              {workspaces.flatMap(ws => ws.boards || []).length === 0 && (
                <div className="col-span-full p-12 border-2 border-dashed border-border-medium rounded-[40px] bg-bg-secondary/30 flex flex-col items-center justify-center gap-4 text-text-tertiary">
              ...
              <section>
                ...
                <div className="space-y-6">
                  {filteredWorkspaces.map(ws => (
                    <div
                      key={ws.id}
                      className="group bg-white border border-border-light rounded-[32px] p-8 flex items-center justify-between shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 cursor-pointer"
                      onClick={() => {
                    ...
                    >
                      <div className="flex items-center gap-8">
                        <div className="w-16 h-16 bg-gradient-to-br from-brand-primary to-brand-secondary rounded-[24px] flex items-center justify-center text-white font-black text-2xl shadow-lg ring-4 ring-bg-secondary group-hover:rotate-6 transition-transform">
                          {ws.name[0]}
                        </div>
                        <div className="space-y-1">
                          <h3 className="text-2xl font-black text-text-primary tracking-tight">{ws.name}</h3>
                          <div className="flex items-center gap-2 text-xs font-bold text-text-tertiary">
                            <span className="text-brand-primary">{ws.boards?.length || 0} Active Projects</span>
            ...
            <aside className="lg:col-span-4 space-y-10">
              <section>
                <div className="flex items-center justify-between mb-8 ps-2">
                  <h2 className="text-xs font-black text-text-tertiary uppercase tracking-[0.3em]">Activity Feed</h2>
                </div>
                <div className="bg-white border border-border-light rounded-[48px] p-10 shadow-sm relative overflow-hidden">
                  {/* Vertical Timeline Line */}
                  <div className="absolute left-[64px] top-24 bottom-24 w-0.5 bg-bg-secondary rounded-full" />

                  <div className="space-y-2 relative z-10">
                    {recentLogs.length === 0 ? (
                      <div className="py-20 text-center opacity-50">
                        <p className="text-xs font-black text-text-tertiary uppercase tracking-widest">No activity yet</p>
                      </div>
                    ) : (
                      recentLogs.map(log => (
                  ...
                  <button 
                    ...
                    className="w-full mt-6 py-4 bg-bg-secondary hover:bg-bg-tertiary rounded-[24px] text-xs font-black uppercase tracking-widest text-text-secondary transition-all active:scale-95 shadow-sm"
                  >
        ...
        {!workspaceSlug && (
          <button
            ...
            className="fixed bottom-12 right-12 w-20 h-20 bg-brand-primary text-white rounded-[32px] shadow-[0_20px_50px_rgba(59,130,246,0.3)] hover:scale-110 active:scale-95 transition-all flex items-center justify-center z-[100] group"
          >
            <Plus size={32} strokeWidth={3} className="group-hover:rotate-90 transition-transform duration-500" />
            <div className="absolute -top-12 right-0 bg-text-primary text-white text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
              New Board
            </div>
          </button>
        )}
          ...
          <section className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-12">
            ...
            <div className="relative bg-gradient-to-br from-blue-600 to-indigo-600 rounded-[48px] p-12 text-white overflow-hidden shadow-2xl shadow-blue-500/20 group">
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                <div className="space-y-4">
                  <div className="px-4 py-1.5 bg-white/10 rounded-full inline-block text-[10px] font-black uppercase tracking-widest border border-white/10">Workspace Active</div>
                ...
                <button 
                  className="bg-white text-blue-600 hover:bg-blue-50 px-10 py-5 rounded-[24px] font-black uppercase tracking-widest text-xs shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center gap-3"
            ...
            <div>
              <div className="flex items-center justify-between mb-10">
                 <h2 className="text-xs font-black text-text-tertiary uppercase tracking-[0.3em]">Workspace Boards</h2>
                 <div className="flex bg-white p-1.5 rounded-2xl border border-border-light shadow-sm">
                    <button className="p-2 bg-bg-secondary rounded-xl text-brand-primary" title="Grid View"><Layout size={16} /></button>
                    <button 
                      onClick={() => navigate(`/w/${workspaceSlug}/team`)}
                      className="p-2 text-text-tertiary hover:text-brand-primary" 
                      title="Team View"
                    >
                      <Users size={16} />
                    </button>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                {filteredWorkspaces.flatMap(ws => ws.boards || []).map(board => (
                  <Link
                    ...
                    className="group bg-white rounded-[40px] border border-border-light shadow-xl shadow-black/[0.02] hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 overflow-hidden flex flex-col"
                  >
                    {/* Card Thumbnail */}
                    <div className="p-3">
                      <div 
                      ...
                      <div className="mt-auto flex items-center justify-between">
                         <div className="flex -space-x-3">
                            {filteredWorkspaces[0]?.workspace_members?.slice(0, 3).map((member, i) => (
                              <div 
                                 key={member.user_id} 
                                 className="w-9 h-9 rounded-full border-4 border-white bg-bg-tertiary overflow-hidden shadow-sm group-hover:scale-110 transition-transform flex items-center justify-center"
                         ...
                         <div className="text-[10px] font-black uppercase tracking-widest text-brand-primary opacity-60 group-hover:opacity-100 transition-opacity">
                            View Board
                         </div>
                      </div>
                    </div>
                ...
                <button
                  ...
                  className="group bg-blue-50 border-2 border-dashed border-blue-200 rounded-[40px] h-full min-h-[400px] flex flex-col items-center justify-center gap-6 hover:bg-blue-600 transition-all duration-500 hover:border-blue-700 text-blue-600 hover:text-white group"
                >
                  <div className="w-20 h-20 rounded-[32px] bg-white text-blue-600 flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform duration-500">
            ...
            <section className="pt-20">
               <h2 className="text-[10px] font-black text-text-tertiary uppercase tracking-[0.4em] mb-10">Recent Activity</h2>
               <div className="space-y-4">
                   {recentLogs.slice(0, showFullHistory ? 20 : 6).map(log => {
                     const formatActivity = (l) => {
                       ...
                       switch (l.action) {
                         ...
                         default: return { text: `${name} updated ${cardName}`, icon: <History size={18} />, color: 'text-brand-primary', bg: 'bg-brand-primary/10' };
                       }
                     };
                     ...
                     return (
                       <div key={log.id} className="flex items-center justify-between p-6 bg-white rounded-[32px] border border-border-light shadow-sm hover:shadow-md transition-shadow">
                          <div className="flex items-center gap-4">
                             <div className={`w-10 h-10 rounded-full ${bg} flex items-center justify-center ${color}`}>
                                {icon}
                             </div>
                             <div className="space-y-0.5">
                                <p className="text-[11px] font-black text-text-primary">{text}</p>
