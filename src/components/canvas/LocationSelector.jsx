    ...
    <div className="w-96 bg-white rounded-[28px] shadow-[0_25px_70px_-15px_rgba(0,0,0,0.4)] border border-border-light overflow-hidden animate-in zoom-in-95 duration-200">
      <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-bg-secondary to-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-black text-foreground">Set Location</h3>
      ...
      <div className="p-4">
        <div className="relative mb-4 group">
          <Search size={16} className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${loading ? 'text-primary animate-pulse' : 'text-text-tertiary'}`} />
          <input
            autoFocus
            className="w-full h-12 bg-bg-secondary border-none rounded-2xl pl-12 pr-4 text-sm font-bold text-foreground outline-none focus:ring-2 focus:ring-primary/20 transition-all placeholder:text-text-tertiary/60"
        ...
        <div className="space-y-2 min-h-[200px] max-h-[400px] overflow-y-auto custom-scrollbar pr-1">
          {results.map(result => (
            ...
            >
              <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-text-tertiary border border-border-light group-hover:bg-primary group-hover:text-white group-hover:border-primary transition-all shrink-0">
                <Navigation size={18} className="group-hover:rotate-45 transition-transform" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-black text-foreground truncate mb-0.5">{result.name}</p>
          ...
          {search && results.length === 0 && !loading && (
            <div className="py-20 text-center opacity-40">
              <p className="text-sm font-bold text-foreground mb-1">No locations found</p>
