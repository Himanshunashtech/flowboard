    ...
    <div className="w-sidebar border-r border-border-light bg-white/40 backdrop-blur-xl flex flex-col h-full relative z-20">
      ...
      <div className="flex-1 overflow-y-auto pt-4">
              ...
              workspaces.map((ws) => (
                <div key={ws.id} className="group px-3">
                  <div className="relative flex items-center">
                    ...
                    <div className="absolute right-5 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                      <button 
                        onClick={(e) => {
                          e.stopPropagation();
                          e.preventDefault();
                        ...
                        className="p-1.5 hover:bg-white rounded-lg text-text-tertiary hover:text-brand-primary"
                        title="Manage Team"
                      >
                         <Users size={14} />
                      </button>
        ...
        <div className="mt-8">
          ...
          <div className="px-4 py-2 text-xs text-text-tertiary italic">No favorites yet</div>
        </div>
      </div>

      <div className="p-4 border-t border-border-light bg-white/50">
        <div className="flex items-center gap-3 p-2 rounded-xl bg-white border border-border-light shadow-sm">
          <div className="w-9 h-9 rounded-lg bg-brand-primary flex items-center justify-center text-white text-xs font-bold shrink-0">
            {user?.email?.substring(0, 2).toUpperCase()}
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-xs font-bold text-text-primary truncate">{profile?.full_name || user?.email?.split('@')[0]}</div>
            <div className="text-[10px] text-text-tertiary truncate">Personal Workspace</div>
          </div>
