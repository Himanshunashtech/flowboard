    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[10000] flex items-center justify-center p-6 animate-in fade-in duration-300">
      ...
      >
        ...
        <div className="p-12 text-center relative z-10">
          ...
          <h2 className="text-3xl font-black text-foreground tracking-tighter mb-4 leading-tight">
            You've been invited!
          </h2>
          
          <p className="text-text-secondary font-medium mb-10 leading-relaxed px-4">
            Join the <span className="text-primary font-black">{workspaceName || 'Project'}</span> team to start collaborating on this board.
          </p>
          ...
          <div className="space-y-4">
            <button 
              onClick={onAccept}
              disabled={loading}
              className="w-full py-5 bg-primary text-white rounded-[24px] font-black uppercase tracking-widest text-xs shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-3 group"
            >
              {loading ? (
            ...
            <button 
              onClick={onReject}
              disabled={loading}
              className="w-full py-5 bg-bg-secondary text-text-tertiary hover:text-foreground rounded-[24px] font-black uppercase tracking-widest text-xs transition-all flex items-center justify-center gap-3"
            >
